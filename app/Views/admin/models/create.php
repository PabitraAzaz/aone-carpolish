...
<?= $this->section('p_name') ?>Car Models|Create<?= $this->endSection() ?>
...
<main>
    ...
    <!-- ⭐ FORM START -->
    <form method="post" action="<?= base_url('admin/models/create') ?>" enctype="multipart/form-data">
        <?= csrf_field() ?>
        ...
        <div class="mb-3">
            ...
            <select name="brand" class="form-control">
                ...
                <?php foreach ($brandD as $catName) : ?>
                    <option value="<?= $catName['brand_id'] ?>" <?= set_select('brand', $catName['brand_id'], False); ?>><?= $catName['name'] ?></option>
                <?php endforeach ?>
            </select>
        </div>
